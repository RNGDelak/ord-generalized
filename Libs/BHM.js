/*
Notation : Bashicu Hyper Matrix (BHM)
Limit : [[Infinity]]
*/
...
window.notation = (() => {
  ...
  function matrix_display(m, mode) {
    ...
    var colsSub = m.map(col => '(' + col.map(n => typeof n === 'number' ? toSubscript(n) : n).join(',') + ')');
    return colsSub.join('');
  }

  // Register Bashicu Hyper Matrix Notation
  register.push({
    id: 'bhm',
  ...
  function fs(ord, n) {
    ...
    return bhmSystem.FS(ord, n);
  }

  function cmp(a, b) {
    return bhmSystem.compare(a, b);
  }

  function isSuccessor(ord) {
    if (ord === Limit || !Array.isArray(ord) || ord.length === 0) return false;
    return !bhmSystem.able(ord);
  }

  function display(ord, mode) {
    return bhmSystem.display(ord, mode);
  }
  ...
  const title = "Bashicu Hyper Matrix Transfinite Number Line";
