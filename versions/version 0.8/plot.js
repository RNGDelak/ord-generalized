...
// config: Unchanged / require user manual changes
let config = {
    aspectratio: 2 / 3,
...
// cam: Camera related variables (update frequently/occasionally)
let cam = {
    w: canvas.width,
    h: canvas.height,
    yStart: 0,
    ...
    fps: undefined,
    times: [],
    lastKeyboardTime: performance.now(), // Track time elapsed between keyboard ticks
    view: {
    ...
    ticks: [],
    impor: [],
    labelsToDraw: [], // Stores computed placement positions for ordinal text labels
    samplerBd: 1e20,
    samplerOrd: null,
    activeKeys: {} // Object to track active state of keys for continuous actions
};

// ==========================================
// LOGIC AND ENGINE FUNCTIONS
// ==========================================
...
function initTicks(width) {
    ...
    cam.labelsToDraw = []; // Reset screen labels array on each compute frame
}
...
// Traversal function targeting label placement
...
// Plot segment traversal function
...
function init() {
    ...
    // Sync keyboard time initialization
...
function clampViewportBounds() {
    ...
    // 1. If we've panned too far right (x0 has passed the right margin threshold)
    if (cam.view.x0 > canvas.width - minOverlap) {
        cam.view.x0 = canvas.width - minOverlap;
        cam.view.x1 = cam.view.x0 + currentWidth;
    }
    // 2. If we've panned too far left (x1 has passed the left margin threshold)
...
window.addEventListener("mousemove", (e) => {
    ...
    // Always enforce the 10% visible rule after mouse drag actions
    clampViewportBounds();
...
window.addEventListener("wheel", (e) => {
    ...
    // Enforce bounds after zoom scaling
    clampViewportBounds();
...
// A lightweight loop to handle smooth, FRAME-RATE INDEPENDENT continuous navigation
function updateKeyboardInput() {
    const now = performance.now();
    let dt = Math.min((now - cam.lastKeyboardTime) / 1000, 0.1); 
    cam.lastKeyboardTime = now;

    // Apply speed modifiers based on Active keys
    ...
    // Linear Pan Scale: pixels per second
    const panSpeed = canvas.width * config.panSpeedBaseFactor * dt;
    
    // Exponential Zoom Scale: zoom multiplier scaled exponentially with delta time
    ...
    // 1. Continuous Panning (Time Scaled)
    ...
    // 2. Continuous Zooming (Exponentially Time Scaled)
    ...
    if (moved) {
        // Enforce boundaries before rendering
        clampViewportBounds();
        render();
    }
