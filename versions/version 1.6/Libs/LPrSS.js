...
function LPrSS_to_Veblen(s, stringify) {
  ...
  if (s.length == 0) {
    ...
    if (zerosplit.length == 1) {
      ...
      for (var i = 0; i < pl; i++) {
        ...
        var inner = LPrSS_to_Veblen(innerseq, false);
      ...
      r = zerosplit.map(e => LPrSS_to_Veblen(e, false));
    }
  }
...
function Veblen_to_LPrSS(s, stringify) {
  if (typeof stringify == "undefined") stringify = true;
  ...
  if (s === 0) {
    ...
    var a = s.map(e => Veblen_to_LPrSS(e, false));
    var r = [];
    ...
    var inner = offsetArray(Veblen_to_LPrSS(innerveblen, false), first);
    var outer = Veblen_to_LPrSS(outerveblen, false).slice(1);
    r = [0];
    r = r.concat(outer);
    r = r.concat(inner);
...
window.notation = (() => {
  ...
  function display(ord, mode) {
    ...
    if (mode == 'CNF')
      return abbreviateVeblen(LPrSS_to_Veblen(ord))
  }
