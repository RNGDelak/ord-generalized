...
// Map of prominent mathematical ordinal structures to their descriptive names
const legendsMap = {
    "Veblen Ordinal": "Veblen Ordinal Space",
    "Buchholz Ordinal": "The Bachmann-Howard / Buchholz Limit Boundary"
};
...
function drawTimelineLabels() {
    ...
    labelsToDraw.forEach((lbl) => {
        const px = lbl.x;
        const py = h * 0.1 + (h * 0.8) * (px / canvas.width) - (h * 0.08); // Follow diagonal line trajectory slightly offset above
...
function render() {
    ...
    const yStart = 0;
    const yEnd = h;
    const tHeight = h * 0.05;
    const ilxw = 1.0 / Math.log(w);
