...
function drawTimelineLabels() {
    ...
    labelsToDraw.forEach((lbl) => {
        ...
        // Check if matching specific notable notation bounds[cite: 2]
        notation.Aliases.forEach(([name, defStr]) => {
            let targetOrd = defStr === "Limit" ? notation.Limit : notation.parse(defStr);
...
function updateKeyboardInput() {
    ...
    const zoomSpeedBase = 0.02;

    // Speed multiplier: Hold 'Shift' to accelerate, hold 'Control' to decelerate
    let multiplier = 1.0;
    if (activeKeys["shift"]) multiplier *= 3.0;
    if (activeKeys["control"]) multiplier *= 0.3;

    const panSpeed = panSpeedBase * multiplier;
    const zoomSpeed = zoomSpeedBase * multiplier;
    ...
    if (activeKeys["arrowdown"]) {
        ...
        if (targetWidth >= maxAllowedWidth) {
            ...
            // Clamp to minimum zoom boundary
            const scaleToLimit = maxAllowedWidth / currentWidth;
            view.x0 = mx + (view.x0 - mx) * scaleToLimit;
            view.x1 = mx + (view.x1 - mx) * scaleToLimit;
        }
