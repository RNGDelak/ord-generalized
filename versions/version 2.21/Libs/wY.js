...
Limit : (0)(1,1,1) in MUTANT Matrix System
*/
...
window.notation = (() => {
  ...
  */
  ...
  function classifyBMSOrdinal(M) {
    ...
    if (cmpBMS(N, [[1, 1, 1]]) >= 0) return "#3f3f3f";
    if (cmpBMS(N, [[1, 1], [2, 2]]) >= 0) return "#000fff";
    if (cmpBMS(N, [[1, 1], [2, 1], [3, 1]]) >= 0) return "#f00fff";
    if (cmpBMS(N, [[1, 1], [2, 1]]) >= 0) return "#00FFF0";
    if (cmpBMS(N, [[1, 1]]) >= 0) return "#00FF00";
  ...
  return {
    ...
    dbmsToBms,
    title,
    classifyBMSOrdinal, trimZeroColumns, DimY_to_DBMS
  };
