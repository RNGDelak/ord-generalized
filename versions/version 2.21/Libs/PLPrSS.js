...
window.notation = (() => {
  ...
  function fs(ord, n) {
    ...
    const lastIdx = len - 1;
    const head = ord[lastIdx];

    if (head === 0) {
      return ord.slice(0, lastIdx);
    }

    const root = P[lastIdx];
    if (root < 0) {
      return ord.slice(0, lastIdx);
    }

    const result = ord.slice(0, lastIdx);
    const part = result.slice(root);
    const offset = head - ord[root] - 1;

    for (let i = 1; i <= n; i++) {
      const curOffset = offset * i;
      for (let j = 0; j < part.length; j++) {
        result.push(part[j] + curOffset);
      }
    }
  ...
  function classifyOrdinal(ord) {
    ...
    if (zeroCount === 1) {
      ...
      // O(1) parent check using P[i] instead of O(N) backward search slices
  ...
  const title = "pointer LPrSS transfinite number line";
